...
class MaskCreator(object):
    ...
    def get_mask(self, shape):
        ...
        mask1=mask.reshape(h,w).astype('int')
        #print(mask1[30:60,30:60])
...
def default_vertices(ax):
    ...
    return ((x1, y1),(x1, (y1+y2)/2), (x1, y2), ((x1+x2)/2, y2),(x2, y2),(x2, (y1+y2)/2),  (x2, y1),  ((x2+x1)/2, y1))
    # return ((x1, y1), (x1,(y1+y2)/4),(x1,(y1+y2)/2), (x1,(3*(y1+y2))/4),(x1, y2),\
    #     ((x1+x2)/4,y2),((x1+x2)/2,y2), ((3*(x1+x2))/4,y2), (x2, y2),\
    #     (x2,(3*(y1+y2))/4),(x2,(y1+y2)/2),(x2,(y1+y2)/4), (x2, y1),\
    #     (3*(x1+x2)/4,y1),((x1+x2)/2,y1),((x1+x2)/4,y1)\
    #     )


def maskImage(img):
    #img = np.array(Image.open('SourceImage.png').convert('RGB'))
    #img = np.array(Image.open('sourceClooney2.jpeg').convert('RGB'))
    ...
    result.save('cropped_image1.bmp')
    #img[mask]=255
    return mask
